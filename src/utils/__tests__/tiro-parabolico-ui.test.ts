...
function setupDOM(): JSDOM {
  ...
  script.textContent = `
    ...
    function calcular() {
      ...
      if (!selectVar || !selectForm || !resultadoDiv) return;
    ...
    if (selectVar && selectForm && inputsCont) {
      selectVar.addEventListener("change", () => {
        ...
        if (formulas[key]) {
          formulas[key].forEach((f, i) => {
            ...
            if (selectForm) selectForm.appendChild(o);
          });
        }
        ocultarTodosInputs();
      });
      ...
      function ocultarTodosInputs() {
        ...
        Array.from(inputsCont.children).forEach(div => {
...
describe('Interfaz de Calculadora de Tiro Parabólico', () => {
  ...
  it('debería mostrar las opciones de variables correctas', () => {
    ...
    if (!selectVariable) {
      throw new Error('selectVariable not found');
    }
  ...
  it('debería cargar las fórmulas al seleccionar una variable', () => {
    const selectVariable = document.getElementById('selectVariable') as HTMLSelectElement;
    const selectFormula = document.getElementById('selectFormula') as HTMLSelectElement;
    
    if (!selectVariable || !selectFormula) {
  ...
  it('debería mostrar los campos de entrada al seleccionar una fórmula', () => {
    ...
    const v0Input = document.querySelector('[data-key="v0"]') as HTMLElement;
    const thetaInput = document.querySelector('[data-key="theta"]') as HTMLElement;
    const tInput = document.querySelector('[data-key="t"]') as HTMLElement;
    
    if (!v0Input || !thetaInput || !tInput) {
  ...
  it('debería calcular y mostrar el resultado correctamente', () => {
    const selectVariable = document.getElementById('selectVariable') as HTMLSelectElement;
    const selectFormula = document.getElementById('selectFormula') as HTMLSelectElement;
    const btnCalcular = document.querySelector('.tiro-button') as HTMLButtonElement;
    
    if (!selectVariable || !selectFormula || !btnCalcular) {
    ...
    const tInput = document.getElementById('t') as HTMLInputElement;
    ...
    const resultadoDiv = document.getElementById('resultado');
    if (!resultadoDiv) {
      throw new Error('resultadoDiv not found');
    }
